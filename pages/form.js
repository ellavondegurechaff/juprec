...
export default function ProfileForm() {
  ...
  const [workPreference, setWorkPreference] = useState(false);
  ...
  const AvailabilityOptions = useMemo(
    () => ["Start Now", "In 1 month", "In 3 months+"],
    []
  );
    ...
    if (selectedOption === "Other") {
      setExpertise(
        expertise.includes("Other")
          ? expertise.filter((opt) => opt !== "Other")
          : [...expertise, "Other"]
      );
    ...
    if (selectedOption === "Other") {
      setInterests(
        interests.includes("Other")
          ? interests.filter((opt) => opt !== "Other")
          : [...interests, "Other"]
      );
  ...
  const uploadFileToSupabase = async (file) => {
    try {
      ...
      const response = await fetch('/api/uploadToSupabase', {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        throw new Error(`Failed to upload file to Supabase: ${errorData.error}`);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const formData = {
        name,
        expertise: expertise.includes("Other")
        ...
        loggedInDiscord: session?.user?.image?.includes("discord")
          ? "true"
          : "false",
        loggedInTwitter: session?.user?.image?.includes("twimg.com")
          ? "true"
          : "false",
      ...
      <div className="max-w-[600px] mx-auto mt-4 p-20 rounded-xl bg-[#1B2229]">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div>
            <label
              htmlFor="expertise"
              className="text-sm text-v2-lily/50 font-medium"
            >
              Areas of Expertise
            </label>
            <div className="mt-1">
          ...
          <div>
            ...
            <div className="mt-1">
              ...
              />
              {errors.talents && (
                <p className="text-red-500 text-sm mt-1">{errors.talents}</p>
              )}
            </div>
          </div>
          <div>
            <label className="text-sm text-v2-lily/50 font-medium">
              Work Preference
            </label>
            ...
            <div className="mt-1 grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="flex items-center">
                ...
                <input
                  ...
                  className="ml-2 block w-full bg-gray-800 border-gray-700 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm px-3 py-2 disabled:opacity-30 disabled:cursor-not-allowed"
                  disabled={
                    session?.user?.image?.includes("twimg.com") || !session
                  }
                />
              </div>
              <div className="flex items-center">
          ...
          <div>
            <label
              htmlFor="previousWorkLinks"
              className="block text-sm font-medium text-gray-300"
            >
              Links to Previous Work (optional)
            </label>
            <div className="mt-1 space-y-4">
              {Array.isArray(previousWorkLinks) &&
                previousWorkLinks.map((link, index) => (
                  <div key={index} className="flex items-center space-x-2">
                    ...
                    {index === previousWorkLinks.length - 1 &&
                      previousWorkLinks.length < 3 && (
                        <button
                          type="button"
              ...
              {!selectedFile ? (
                <>
                  ...
                  <p className="mt-3 text-center text-v2-lily/75 text-sm">
                    {isDragActive ? (
                      ...
                      <>
                        <b className="font-semibold text-v2-lily">
                          Click to upload
                        </b>{" "}
                        or drag and drop
                        <br />
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between bg-gray-800 px-4 py-2 rounded-md w-full">
                    <span className="text-v2-lily truncate">
                      {selectedFile.name}
                    </span>
                    <button
                      type="button"
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-gray-800 rounded-lg p-8 max-w-md mx-auto text-center">
            ...
            <p className="text-gray-300 mb-6">
              Please log in with Discord to submit the form.
            </p>
