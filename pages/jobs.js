...
export default function JobDetails() {
  ...
  const uploadFileToSupabase = async (file, positionName) => {
    try {
      const formData = new FormData();
      formData.append('file', file);
      formData.append('positionName', positionName); // Add position name to form data
  
      const response = await fetch('/api/uploadtoSupabaseJobs', {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        throw new Error(`Failed to upload file to Supabase Storage: ${errorData.error}`);
      }
    } catch (error) {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      if (selectedFile && selectedJob.title) {
        await uploadFileToSupabase(selectedFile, selectedJob.title);
      }
  ...
  return (
    ...
    >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {jobs.map((job) => (
            ...
            >
              ...
              <h2 className="text-xl font-bold mb-2 text-v2-lily">
                {job.title}
              </h2>
      ...
      {selectedJob && (
        <div
          className={`fixed inset-0 flex items-center justify-center backdrop-blur-md bg-opacity-75 transition-opacity ${
          ...
          <div className="bg-[#1B2229] rounded-lg w-full max-w-lg p-8 mx-4 transition-transform duration-300 ease-in-out transform hover:scale-105 overflow-y-auto max-h-[80vh]">
            ...
            <div className="mb-6">
              ...
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: selectedJob.company.description,
                }}
              />
            </div>
            <div className="space-y-4">
              <div>
                <label
                  htmlFor="name"
                  className="block text-xs font-medium text-v2-lily/50"
                >
                  Name
                </label>
                <input
              ...
              <div>
                <label
                  htmlFor="resume"
                  className="block text-xs font-medium text-v2-lily/50 mb-1"
                >
                  Resume / Portfolio
                </label>
                <div
            ...
            {!session && (
              <p className="text-red-500 text-xs mt-4 text-center">
                Please log in to apply for this workgroup
              </p>
            )}
          </div>
        </div>
